...
void init(int n){
    ...
    rep(i, 1, n) sum[i] = (sum[i - 1] + 1LL * mu[i] * i * i % MOD) % MOD;
}

inline LL gx(LL n, LL x){return n / x ? n / (n / x) : inf;}
inline LL S2(LL n){return n * (n + 1) % MOD * (2 * n + 1) % MOD * inv6 % MOD;}
...
LL f(int n, int m){
    ...
    for(LL l = 1, r; l <= up; l = r + 1){
        ...
        ret += (sum[r] - sum[l - 1] + MOD) *  //注意mu(i) * i^2 的和不能用两个前缀和直接相乘
