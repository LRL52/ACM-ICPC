...
//为啥要保证G数组是清空的啊，为啥每次做完了G数组要清空
//为啥tot < n不行啊，都是在mod x^n下运算
void PolyInv(LL *F, LL *G, int n){
    ...
    //for(int i = 0; i < tot; ++i) printf("%lld ", G[i]);
    //puts("");
}

/*void PolyInv(LL *a, LL *b, int n){ //A(x)^(-1) mod x^n, 答案写入b, a保持不变
    b[0] = ksm(a[0], MOD - 2);*/
