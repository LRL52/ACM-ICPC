...
signed main() {
#ifdef LRL52
	freopen("P5273_2.in", "r", stdin);
    freopen("P5273.out", "w", stdout);
#endif
	int n; LL K = 0, K1 = 0, K3 = 0; scanf("%d%s", &n, s + 1);
    int len = strlen(s + 1);
    rep(i, 1, len){
        K = (K * 10LL + (s[i] - '0')) % MOD;
        K1 = (K1 * 10LL + (s[i] - '0')) % (MOD - 1);
    }
    rep(i, 1, min(6, len)) K3 = K3 * 10LL + (s[i] - '0');
    for(int i = 0; i < n; ++i) scanf("%d", &a[i]);
    if(K3 >= n && a[0] == 0) for(int i = 0; i < n; ++i) b[i] = 0;
    else PolyPow2(a, K, b, n, K1);
