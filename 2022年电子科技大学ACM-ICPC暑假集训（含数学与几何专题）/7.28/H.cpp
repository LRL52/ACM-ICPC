...
int n, m, instk[M << 1], f[M << 1], vis[M << 1], dcnt, g[N];
struct Edge{
    int v, nxt;
}e[M << 1];
int head[N], et = 1;
...
int dfs(int lst, int x, int k){
    ...
    head[x] = 0;
    if(eid) e[eid].nxt = head[x], head[x] = eid;
    if(maxdid) e[maxdid].nxt = head[x], head[x] = maxdid;
    f[k] += max(maxd, g[x]), instk[k] = 0;
