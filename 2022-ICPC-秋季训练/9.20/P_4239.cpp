...
using namespace std;
const int N = 1e3 + 55, inf = 0x3f3f3f3f; char Buf[1 << 21], *_A = Buf, *_B = Buf, cc;
...
signed main() {
    ...
    int lim = n + 1;
    Poly1.PolyInv(a, Ans1, lim);
    Poly2.PolyInv(a, Ans2, lim);
    Poly3.PolyInv(a, Ans3, lim);
    for(int i = 0; i < n; ++i) printf("%lld ", CRT(i));
    return 0;
}